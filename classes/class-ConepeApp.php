<?php
/**
 * ConepeApp - Gerencia Models, Controllers e Views
 *
 * @package ConepeApp
 * @since 0.1
 */
class ConepeApp
{
	 ...
	 * $menu
	 *
	 * Receberá o valor do menu (Vindo da URL).
	 * exemplo.com/controlador/mnu...
	 *
	 * @access private
	 */
	private $menu;
	...
	public function __construct () {
		...
		if (! $this->menu ) {$this->menu = 'principal';}
		 ...
		 */
			if ( ! $this->controlador ) {
			    ...
			    // Altera o Menu pai para o Corrente
			    $this->controlador->menu_pai = $this->controlador->menu;

				// Altera o Menu corrente	
				$this->controlador->menu = $this->menu;
		...
		if ( ! file_exists( ABSPATH . '/controllers/' . $this->controlador . '.php' ) ) {
			...
			echo "<h3> nao encontrei o controller " . $this->controlador . "</h3></br>";
...
//			foreach ( $path as $key => $value ) {echo $key . " - " . $value . " cada path </br>";}
/*
			echo $path[0] . " = path[0]</br>";
			echo $path[1] . " = path[1]</br>";
			echo $path[2] . " = path[2]</br>";
*/			
			...
			if (substr($this->acao,0,3) == 'mnu') {
    			$this->menu = substr($this->acao,3);
				$this->acao = null;
			}
